...
def uv_group_node_set_defaults(
    ...
    set_value(
        node=node_group,
        sock_name="Rotation",
        sock_bl_idname_expected=get_rotation_bl_idname(),
        value=rotation)
    set_value(
        node=node_group,
...
def prepare_simple_uv_group_node(
    ...
    create_node_socket(
        node_group,
        socket_type=get_rotation_bl_idname(),
        in_out="INPUT",
        name="Rotation")
    create_node_socket(
        node_group,
...
def get_rotation_bl_idname() -> str:
    """ The bl_idname for Rotation socket changed from NodeSocketFloatAngle to NodeSocketFloatAngle """
    if bpy.app.version >= (4, 0):
