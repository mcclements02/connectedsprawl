...
from .modules.poliigon_core.api import ApiStatus
...
def build_ui(ui, context):
    ...
    # In case of no internet, we cannot determine this, so skip showing the survey.
    if all(
        [
            cTB._api.status != ApiStatus.NO_INTERNET,
            cTB.user.user_profile is None,
            cTB.did_show_profile_survey is False,
        ]
    ):
