...
def disable_half_float_precision(img: bpy.types.Image) -> None:
    """Attempts to disable half precision, resulting in low quality rendering

    Seems to be a problem in Blender 5 when loading tiff format maps.
    """
    if not img.filepath:
        return
    _, ext = os.path.splitext(img.filepath)
    if ext.lower() not in {".tif", ".tiff"}:
        return
    if not hasattr(img, "use_half_precision"):
        return
    try:
