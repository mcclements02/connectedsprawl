...
from ..props import PoliigonObjectProps
...
from ..panels.panel_model import _change_multiple_models_tex, get_tex_maps_and_asset_name
from .utils_operator import fill_size_dropdown
...
class POLIIGON_OT_model(Operator):
    ...
    def execute(self, context):
        ...
        if (not self.do_use_collection) or (not coll_exists):
        ...
        empty_parent = None
        if did_fresh_import:
            empty_parent = self.setup_empty_parent(context, new_objs, asset_name)
            if blend_import and not self.do_use_collection:
                empty_parent.location = bpy.context.scene.cursor.location
        ...
        if self.do_use_collection:
            ...
            if did_fresh_import:
                ...
                for _obj in [empty_parent] + new_objs:
            ...
            self.append_cleanup(context, empty_parent)

        # A model will often be imported with the default size instead of user specified size
        # This is to change the size of every material imported to the user defined one
        self.fix_material_import_size(context)
    ...
    def run_fresh_import(self,
        ...
        for path_proj in project_files:
            cTB.logger.debug(f"Importing project file: {path_proj}")
            ...
            if not f_Ex(path_proj):
                ...
                cTB.logger.error(f"Model file missing: {path_proj}")
                continue
    ...
    def set_poliigon_props_model(
            self, obj: bpy.types.Object, lod: str, is_parent: bool = False) -> None:
        """Sets Poliigon properties on an imported object."""
        props: PoliigonObjectProps = obj.poliigon_props
        props.asset_id = self.asset_data.asset_id
        props.asset_name = self.asset_data.asset_name
        props.asset_type = self.asset_data.asset_type.name
        props.size = self.size
        if lod is not None:
            props.lod = lod
        props.use_collection = self.do_use_collection
        props.link_blend = self.do_link_blend
        props.is_parent = is_parent
    ...
    def setup_empty_parent(self,
        ...
        if self.do_use_collection:
            ...
            empty_obj.empty_display_size = radius * 0.5

        layer = context.view_layer.active_layer_collection
        layer.collection.objects.link(empty_obj)
        ...
        for _mesh in new_meshes.copy():
            if _mesh.type == "EMPTY" and not self.object_has_children(_mesh):
                ...
                continue
            _mesh.parent = empty_obj

        self.set_poliigon_props_model(empty_obj, None, is_parent=True)
        return empty_obj
    ...
    def fix_material_import_size(self, context) -> None:
        """ Make sure models have a material of the size specified by user """
        old_size = self.get_default_size(context)
        if not old_size:
            return
        tex_maps, _ = get_tex_maps_and_asset_name(self.asset_id, self.size)
        _change_multiple_models_tex(context, context.scene.objects, tex_maps, self.asset_id, old_size, self.size)

    def get_default_size(self, context) -> Optional[str]:
        """ Find which size was applied to the materials by default """
        asset_data = cTB._asset_index.get_asset(self.asset_id)
        asset_type_data = asset_data.get_type_data()

        # The default size will for sure be one of the local sizes
        ...
        for _obj in bpy.context.selected_objects:
            if not (hasattr(_obj, "poliigon_props") and _obj.active_material):
                continue

            _props = _obj.poliigon_props
            if not _props.asset_id == self.asset_id:
                continue

            if not _obj.active_material.node_tree:
                continue

            nodes = _obj.active_material.node_tree.nodes
            result = get_size_from_nodes(nodes, local_sizes, check_nested_nodes=True)
            if result:
                return result
...
def get_size_from_nodes(nodes: List[bpy.types.Node],
                        local_sizes: List[str],
                        check_nested_nodes: bool = False) -> Optional[str]:
    """ Check all nodes in a material (and optionally also the nested nodes)
    to find what the size is """
    for _node in nodes:
        if _node.type == "TEX_IMAGE":
            for _size in local_sizes:
                if _size in _node.image.name:
                    return _size
        elif hasattr(_node, "node_tree"):
            if check_nested_nodes:
                result = get_size_from_nodes(_node.node_tree.nodes, local_sizes)
                if result:
                    return result
    return None
