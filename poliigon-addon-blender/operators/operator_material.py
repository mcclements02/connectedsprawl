...
def apply_subdivision(obj: bpy.types.Object) -> None:
    """ Add a subdivision modifier to the object, and if Cycles render, add adaptive subdiv """
    ...
    if bpy.context.scene.render.engine == "CYCLES":
        if bpy.app.version < (5, 0):
            ...
            mod_subdiv.use_adaptive_subdivision = True
...
class POLIIGON_OT_material(Operator):
    ...
    def _inherit_settings_from_active_mat(self, context) -> None:
        """Pre-fill operator props from the currently-applied Poliigon material
        so that a swap defaults to the previous material's settings."""
        mat = None
        obj = context.object
        if obj is not None and hasattr(obj.data, "materials"):
            ...
            for sel_obj in context.selected_objects:
                if not hasattr(sel_obj.data, "materials"):
                    continue
                mat = sel_obj.active_material
                break

        if mat is None or mat.poliigon_props.asset_id == -1:
            return

        props = mat.poliigon_props
        self.scale = props.scale
        self.mapping = props.mapping
        self.mode_disp = props.mode_disp
        self.displacement = props.displacement
        self.use_16bit = props.use_16bit
        cTB.logger.debug("POLIIGON_OT_material inherited settings from active material: "
                         f"mapping {self.mapping}, scale {self.scale}, "
                         f"use_16bit {self.use_16bit}, mode_disp {self.mode_disp}, "
    ...
    def execute(self, context):
        # Inherit mapping/scale/etc from the active Poliigon material so that
        # swapping a material keeps the previous settings.
        # Guard with exec_count so undo-panel re-executions don't override the
        # user's property changes with stale active-material values.
        if self.exec_count == 0:
            self._inherit_settings_from_active_mat(context)
        ...
        for _obj in objs_add_disp:
            apply_subdivision(_obj)
