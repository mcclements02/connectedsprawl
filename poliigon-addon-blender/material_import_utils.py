...
COMPATIBLE_SOCKET_TYPES = {"NodeSocketFloat", "NodeSocketFloatAngle"}
...
def create_link(
    *,
    ...
    # Allow float and float angle sockets to be interchangeable, as they are
    # functionally equivalent and cause issues between Blender 3.x and 4.x.
    is_compatible_out = _are_socks_compatible(sock_out.bl_idname, sock_out_bl_idname_expected)

    if not is_compatible_out:
    ...
    is_compatible_in = _are_socks_compatible(sock_in.bl_idname, sock_in_bl_idname_expected)

    if not is_compatible_in:
...
def _are_socks_compatible(
    bl_idname_to_check: str,
    bl_idname_expected: str,
) -> bool:
    """ Check if a sock is compatible with the expected one """
    if bl_idname_to_check == bl_idname_expected:
        return True

    condition1 = bl_idname_to_check in COMPATIBLE_SOCKET_TYPES
    condition2 = bl_idname_expected in COMPATIBLE_SOCKET_TYPES
    are_compatible = condition1 and condition2
...
def set_value(
    *,
    ...
    # Allow float and float angle sockets to be interchangeable, as they are
    # functionally equivalent and cause issues between Blender 3.x and 4.x.
    is_compatible = _are_socks_compatible(sock.bl_idname, sock_bl_idname_expected)
