...
class FilterOptions(enum.Enum):
    ...
    RECENTLY_VIEWED_ASSETS = _m("Recently Viewed")
    OWNED_ASSETS = _m("Owned")
    RECENT_DOWNLOADED_ASSETS = _m("Previous Downloads")
    ...
    @classmethod
    def get_list_for_user(cls, user: PoliigonUser) -> List[enum.Enum]:
        filter_list = cls.get_list()
        ...
        if user.count_assets_owned is not None and user.count_assets_owned == 0:
            filter_list.remove(cls.OWNED_ASSETS)
        if user.count_assets_recent_viewed == 0:
            filter_list.remove(cls.RECENTLY_VIEWED_ASSETS)
        return filter_list

    @classmethod
    def get_default_filter(cls,
                           user: PoliigonUser,
                           addon_last_view: datetime
                           ) -> enum.Enum:
        if user is None or user.last_poliigon_view is None:
            return cls.ALL_ASSETS
        if addon_last_view is None and user.count_assets_recent_viewed:
            return cls.RECENTLY_VIEWED_ASSETS
        if user.last_poliigon_view > addon_last_view:
            return cls.RECENTLY_VIEWED_ASSETS
        return cls.ALL_ASSETS
    ...
    def get_from_query(cls, key_query: str) -> Optional[Any]:
        ...
        elif key_query == KEY_TAB_RECENTLY_VIEWED:
            return cls.RECENTLY_VIEWED_ASSETS
        else:
            return None
    ...
    def map_to_query(self) -> Optional[str]:
        ...
        elif self == self.RECENTLY_VIEWED_ASSETS:
            return KEY_TAB_RECENTLY_VIEWED
        else:
            return None
