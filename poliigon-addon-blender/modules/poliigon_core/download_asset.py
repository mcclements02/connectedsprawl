...
class AssetDownload:
    ...
    def check_download_progress(self) -> None:
        ...
        for download in self.download_list:
            ...
            exc = fut.exception()
            if exc is not None:
                res = ApiResponse(ok=False, body={}, error=str(exc))
    ...
    def set_progress(self) -> None:
        ...
        prev_progress = self.asset_data.state.dl.get_progress()
        new_progress = max(progress, 0.001)
        self.asset_data.state.dl.set_progress(new_progress)
        self.asset_data.state.dl.set_downloaded_bytes(self.size_asset_bytes_downloaded)
        ...
        # Update download callback only if the progress was bigger than 1%
