...
def run_threaded(
        ...
        max_threads: Optional[int] = None,
        foreground: bool = False) -> Callable:
    """Decorator to schedule a method for threaded execution via self._tm.

    IMPORTANT: This method expects the decorated method to belong to a class with a
    `_tm` attribute of type ThreadManager. Falls back to a daemon thread if `_tm` is None.
    ...
    def wrapped_func(func: Callable) -> Callable:
        ...
        def wrapped_func_call(self, *args, **kwargs):
            ...
            if self._tm is not None:
                return self._tm.queue_thread(
                    func, key_pool, max_threads, foreground,
                    *args, **kwargs)
            else:
                threading.Thread(
