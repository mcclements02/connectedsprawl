...
MAP_DESCRIPTIONS: Dict = {
    ...
    MapType.IDMAP: MapDescription(
        description=_t(
            "ID Map assigns different colours to different parts of your model, "
            "which is then used to mask your layers in different ways"),
        display_name=_t("ID Map")
    ),

    MapType.EMISSION: MapDescription(
        description=_t(
            "Emissive Maps define areas of a surface that emit light independently of external lighting conditions"),
        display_name=_t("Emission")
    )
}
