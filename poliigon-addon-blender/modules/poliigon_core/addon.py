...
from .thread_manager import run_threaded, PoolKeys, ThreadManager
from .asset_cache import AssetCacheSystem
from .user import PoliigonUser


DIR_PATH = os.path.dirname(os.path.abspath(__file__))
RESOURCES_PATH = os.path.join(DIR_PATH, "resources")
CACHE_TIME_LIMIT_VALIDATION = 7 * 24 * 60 * 60  # 7 days
MAX_CACHE_VALIDATION_ATTEMPTS = 3
...
class PoliigonAddon():
    ...
    all_assets_failed: bool = False

    # Cache system
    ignore_all_assets_cache: bool = False
    startup_cache_load_executed: bool = False
    all_assets_cache_refreshing: bool = False
    cache_validation_attempts_left: int
    asset_cache: AssetCacheSystem
    ...
    def __init__(self,
        ...
        self.logger_asset_cache = self.log_manager.initialize_logger(
            "ASSET_CACHE", have_filehandler=have_filehandler)
        ...
        self.logger.debug(f"API URL V2: {self._api.api_url}")
        ...
        self._tm = ThreadManager(callback_print_exc=callback_print_exc)
        ...
        self.asset_cache = AssetCacheSystem(
            cache_dir=os.path.join(self.user_addon_dir, "cache"),
            max_cache_age=CACHE_TIME_LIMIT_VALIDATION,
            logger=self.logger_asset_cache,
            tm=self._tm,
        )
        self.cache_validation_attempts_left = MAX_CACHE_VALIDATION_ATTEMPTS
    ...
    def reset_cache_validation_attempts_left(self):
        """Resets the cache validation tries to default value."""
        self.cache_validation_attempts_left = MAX_CACHE_VALIDATION_ATTEMPTS
    ...
    def refresh_top_level_queries_flags(self):
        ...
        self.all_assets_failed = False
        self.all_assets_cache_refreshing = False
    ...
    def update_user_from_dict(self, user_resp_dict: Dict):
        if not self.user:
            self.user = PoliigonUser.create_from_dict(user_resp_dict)
        else:
            self.user.update_from_dict(user_resp_dict)
