...
REGEX_DATETIME_ISOFORMAT = re.compile(REGEX_DATETIME_ISOFORMAT)
...
class AssetIndex():
    ...
    # Cache for _decode_tex_convention_1 results, keyed by
    # ((map_type, file_formats), resolutions, creation_method, width, height).
    # Avoids re-instantiating TextureMapDesc objects for assets that share
    # the same map/resolution/specs layout, which is the case for the
    # vast majority of assets (~99% hit rate on a 15k-asset catalogue).
    _decode_conv1_cache: Dict[tuple, any]
    # Cache for _decode_render_schema_tex results, keyed by
    # (map_codes_per_workflow, resolutions).
    # Same rationale as _decode_conv1_cache for convention 0 assets
    # (~96% hit rate on a 15k-asset catalogue).
    _decode_conv0_cache: Dict[tuple, any]
    ...
    # Staging sets for asset operations requested before the asset index is ready.
    # If an asset ID is not yet present in the local index, it is queued here and
    # processed once the Get All Assets request completes and populates the index.
    _staging_lock: Lock = Lock()
    _staging_mark_purchased: Set[int] = set()
    _staging_mark_recent_downloaded: Set[int] = set()
    _staging_update_local_assets: Set[int] = set()
    ...
    def __init__(self,
        ...
        self._decode_conv1_cache = {}
        self._decode_conv0_cache = {}
    ...
    def _get_api_time(self, data: Dict, key: str) -> float:
        ...
        if t is None:
            ...
            return 0.0
        if not REGEX_DATETIME_ISOFORMAT.match(t):
            # TODO(Andreas): Not sure how we would want to report this, here
            return 0.0
    ...
    def _check_cache_decode_results(self, asset_dict: Dict, convention: int):
        """Returns a previously cached texture decode result, or None on a miss.

        Builds a cache key from the fields that fully determine the decode
        output for the given convention, then looks up the appropriate cache.
        Returns None if no entry exists, signalling the caller to run the
        full decode and store the result via _store_cache_decode_results.
        """
        if convention == 1:
            asset_maps = asset_dict.get("Maps", [])
            _specs = asset_dict.get("Specifications", {})
            _phys = _specs.get("physical_size_cm", {})
            key = (
                tuple(
                    (m.get("type", "UNKNOWN"), tuple(m.get("file_formats", [])))
                    for m in asset_maps
                ),
                tuple(asset_dict.get("Resolutions", [])),
                _specs.get("creation_method", ""),
                _phys.get("width", None),
                _phys.get("height", None),
            )
            return self._decode_conv1_cache.get(key)
        else:
            _maps_raw = asset_dict.get("Maps", [])
            key = (
                tuple(
                    (m.get("workflow", "REGULAR"), tuple(m.get("maps", [])))
                    for m in _maps_raw
                ),
                tuple(asset_dict.get("Resolutions", [])),
            )
            return self._decode_conv0_cache.get(key)

    def _store_cache_decode_results(self, asset_dict: Dict, convention: int, result) -> None:
        """Stores a texture decode result in the appropriate session cache.

        Should be called after a full decode so subsequent assets with the
        same map/resolution/specs layout can skip the decode entirely.
        The stored result is shared by reference — callers must not mutate
        the returned objects (TextureMapDesc instances, size lists, etc.).
        """
        if convention == 1:
            asset_maps = asset_dict.get("Maps", [])
            _specs = asset_dict.get("Specifications", {})
            ...
            self._decode_conv1_cache[key] = result
        else:
            _maps_raw = asset_dict.get("Maps", [])
            key = (
                tuple(
            ...
            self._decode_conv0_cache[key] = result
    ...
    def _decode_tex_convention_1(
        ...
        _cached = self._check_cache_decode_results(asset_dict, convention=1)
        ...
        _result = maps_dictionary, sizes, dimensions, creation_method
        self._store_cache_decode_results(asset_dict, convention=1, result=_result)
        return _result
    ...
    def _decode_render_schema_tex(self,
        ...
        _cached = self._check_cache_decode_results(asset_dict, convention=0)
        if _cached is not None:
            return _cached
        ...
        if len(all_variants) > 0:
            ...
            all_variants = None
        _result = (tex_desc_dict, all_sizes, all_variants)
        self._store_cache_decode_results(asset_dict, convention=0, result=_result)
        return _result
    ...
    def mark_purchased(self, asset_id: int) -> None:
        ...
        with self._staging_lock:
            if asset_id not in self.all_assets:
                self._staging_mark_purchased.add(asset_id)
                return
            if asset_id in self._staging_mark_purchased:
                self._staging_mark_purchased.remove(asset_id)
    ...
    def mark_recent_downloaded(self, asset_id: int) -> None:
        ...
        with self._staging_lock:
            if asset_id not in self.all_assets:
                self._staging_mark_recent_downloaded.add(asset_id)
                return
            if asset_id in self._staging_mark_recent_downloaded:
                self._staging_mark_recent_downloaded.remove(asset_id)
    ...
    def _analyze_single_file(self,
        ...
        if base_filename.startswith("._"):
            # Ignore metadata files generated by OSX when accessing FAT formatted drives
            return 0
    ...
    def update_all_local_assets(self,
        ...
        with self._staging_lock:
            _asset_ids = set(asset_id_list)
            _all_assets = set(self.all_assets)
            _missing_assets = _asset_ids - _all_assets
            self._staging_update_local_assets = _missing_assets
            asset_id_list = list(_asset_ids & _all_assets)
    ...
    def populate_assets(self,
        ...
        for asset_dict in asset_dict_list:
            try:
                # Check for override before constructing AssetData
                # Can be convention 0 or 1, hence two key types.
                asset_id = asset_dict.get("AssetID")
                if asset_id is None:
                    asset_id = asset_dict.get("id")
                override_data = self._get_asset_override(asset_id)
                if override_data is not None:
                    asset_dict = override_data
    ...
    def clean_up_invalid_queries(self) -> None:
        """Removes invalid queries with asset IDs not in all_assets."""
        invalid_queries = []
        for query_tuple in self.cached_queries:
            asset_ids = self.cached_queries[query_tuple]
            valid_asset_ids = [
                asset_id for asset_id in asset_ids
                if asset_id in self.all_assets
            ]
            if len(valid_asset_ids) == 0:
                invalid_queries.append(query_tuple)
            else:
                self.cached_queries[query_tuple] = valid_asset_ids
        for query_tuple in invalid_queries:
            self.logger.debug(f"Cleaning up invalid query: {query_tuple}")
            del self.cached_queries[query_tuple]

    def process_staged_asset_operations(self, library_dirs: List[str]) -> None:
        """Apply all staged asset operations."""
        with self._staging_lock:
            _staged_purchased = list(self._staging_mark_purchased)
            _staged_recent = list(self._staging_mark_recent_downloaded)
            _staged_local = list(self._staging_update_local_assets)

        for asset_id in _staged_purchased:
            if asset_id in self.all_assets:
                self.mark_purchased(asset_id)
        for asset_id in _staged_recent:
            if asset_id in self.all_assets:
                self.mark_recent_downloaded(asset_id)
        if _staged_local:
            self.update_all_local_assets(
                library_dirs,
                asset_id_list=_staged_local
            )
    ...
    def load_cache(self, use_gzip: bool = True) -> None:
        ...
        for asset_dict in asset_list:
            ...
            assign_data = asset_dict
            override_data = self._get_asset_override(asset_id)
            if override_data is not None:
                # Use construct_asset for API-format override data
                # Preserve purchased state from cached asset
                purchased = asset_dict.get("is_purchased")
                assign_data = self.construct_asset(override_data, purchased)
            else:
                # Use _from_dict for cache-format data
                assign_data = assets.AssetData._from_dict(asset_dict)

            self.all_assets[assign_data.asset_id] = assign_data

        self._verify_cache()

    def _get_asset_override(self, asset_id: int) -> Optional[Dict]:
        """Get override data for an asset if available."""
        if not asset_id:
            return None
        if not self.addon or not hasattr(self.addon, '_api'):
            return None
        override_data = self.addon._api.get_asset_override(asset_id)
        if override_data:
            self.logger.debug(f"Applied data override to asset {asset_id}")
    ...
    def flush(self, all_assets: bool = False, staging: bool = False) -> None:
        """Flushes the query cache.

        Args:
        all_assets: If True, not only the query cache,
                    but the entire AssetIndex gets flushed.
        staging: If True, also clears all staging areas.
        """
        ...
        if all_assets:
            self.all_assets = {}
        if staging:
            self._staging_mark_purchased.clear()
            self._staging_mark_recent_downloaded.clear()
            self._staging_update_local_assets.clear()
