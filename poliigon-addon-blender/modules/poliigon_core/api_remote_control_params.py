...
from .api import PoliigonConnector
...
class ApiJobParamsLogin(ApiJobParams):
    ...
    def _exec_job_login_browser(self, api_rc, job) -> None:
        ...
        Gets called in thread (thread_exec_job_login).
        """
        _api: PoliigonConnector = api_rc._api
        job.result = _api.log_in_with_website()

        if not job.result.ok:
            return

        job.result = _api.poll_login_with_website_success(
            timeout=300,
    ...
    def finish(self, api_rc, job) -> None:
        ...
        if not job.result.ok or self.mode == CmdLoginMode.LOGOUT:
            ...
            api_rc._addon.user = PoliigonUser.create_from_dict(job.result.body)
...
class ApiJobParamsGetUserData(ApiJobParams):
    ...
    def finish(self, api_rc, job) -> None:
        ...
        api_rc._addon.user.update_from_dict(job.result.body)
        ...
        if not self.do_fetch_asset_data:
            return

        _page_size = addon_params.online_assets_chunk_size
        _library_paths = api_rc._addon.get_library_paths()

        api_rc._addon.update_user_from_dict(job.result.body)
        ...
        if api_rc._addon.user_legacy_own_assets():
            ...
            _page_size = addon_params.my_assets_chunk_size
            api_rc.add_job_get_assets(
                library_paths=_library_paths,
...
class ApiJobParamsGetAvailablePlans(ApiJobParams):
    ...
    def thread_execute(self, api_rc, job) -> None:
        ...
        _paused_or_cancelled = (
            SubscriptionState.PAUSED,
            SubscriptionState.PAUSE_SCHEDULED,
            SubscriptionState.CANCELLED,
        )
        if api_rc._addon.user.plan.subscription_state in _paused_or_cancelled:
            api_rc.logger.debug("Skipping fetch of available plans for paused/cancelled user.")
            job.ok = True
            job.result = ApiResponse(ok=True, body={"data": []}, error="")
            return
...
class ApiJobParamsGetAssets(ApiJobParams):
    ...
    def _finish_toplevel_requests(self, api_rc, job) -> List[int]:
        ...
        if self.tab == KEY_TAB_MY_ASSETS or self.tab == KEY_TAB_RECENT_DOWNLOADS:
            ...
            if api_rc._addon.all_assets_fetched:
                asset_ids_resp = self._filter_by_known_asset_ids(api_rc, asset_ids_resp)
    ...
    def _validate_all_assets_cache(self, api_rc, job) -> None:
        """Validates all assets cache before proceeding
        if not valid, refetch all assets
        """
        if api_rc._addon.cache_validation_attempts_left <= 0:
            api_rc.logger.debug("Skipping cache validation, tries exhausted.")
            return
        _is_cache_valid = api_rc._addon.asset_cache.validate_all_assets_cache(
            self._get_asset_ids_from_body(job)
        )
        if _is_cache_valid:
            # If cache is valid, reset tries counter
            api_rc.logger.debug("Cache is valid, resetting attempts left.")
            api_rc._addon.reset_cache_validation_attempts_left()
            return
        if (
            api_rc._addon.all_assets_cache_refreshing or not api_rc._addon.all_assets_fetched
        ):
            api_rc.logger.debug(
                "Cache is already being refreshed or not yet fetched, skipping get all assets call."
            )
            return

        # If cache is not valid, refetch all assets
        api_rc.logger.debug("Cache is invalid, refetching all assets.")
        api_rc._addon.all_assets_cache_refreshing = True
        api_rc._addon.cache_validation_attempts_left -= 1
    ...
    def finish(self, api_rc, job) -> None:
        ...
        if not api_rc._addon.ignore_all_assets_cache:
            self._validate_all_assets_cache(api_rc, job)
...
class ApiJobParamsGetAllAssets(ApiJobParams):
    ...
    using_cached_all_assets: bool = False

    POOL_KEY = PoolKeys.INTERACTIVE

    def __eq__(self, other):
        return self.force_request == other.force_request

    def __str__(self) -> str:
        # No library paths in here, as the string might be reported to server!
        return (
            "ApiJobParamsGetAllAssets - "
            f"force: {self.force_request}"
            f"asset_id_list: {self.asset_id_list}"
        )
    ...
    def thread_execute(self, api_rc, job) -> None:
        ...
        started from thread_schedule.
        """
        _addon = api_rc._addon

        _addon.all_assets_fetched = False
        if api_rc.in_shutdown:
            job.result = get_shutdown_response()
            return

        query_exists = api_rc._asset_index.query_exists(
            self.all_assets_key, IDX_PAGE_ACCUMULATED, PAGE_SIZE_ACCUMULATED
        )
        if query_exists and not self.force_request:
            self.already_in_index = True
            job.result = ApiResponse(
                ok=True, body={"data": []}, error="job cancelled, query exists"
            )
            return
        # Try to load All Assets from cache on first attempt in the session (startup)
        if not _addon.ignore_all_assets_cache and not _addon.startup_cache_load_executed:
            api_rc.logger.debug("Loading All Assets from cache on startup.")
            _addon.startup_cache_load_executed = True
            cached_all_assets = _addon.asset_cache.read_all_assets_cache()
            if cached_all_assets is not None and not self.force_request:
                self.using_cached_all_assets = True
                job.result = ApiResponse(
                    ok=True, body=cached_all_assets, error=""
                )
                return
    ...
    def finish(self, api_rc, job) -> None:
        ...
        if not job.result.ok:
            api_rc._addon.all_assets_failed = True
            return
        ...
        # Write all assets to cache if we fetched them from API
        if not api_rc._addon.ignore_all_assets_cache and not self.using_cached_all_assets:
            api_rc._addon.asset_cache.write_all_assets_cache(job.result.body)
            api_rc._addon.all_assets_cache_refreshing = False
        ...
        # Handles pending asset ID processing that depends on a fully populated Asset Index.
        api_rc._asset_index.process_staged_asset_operations(self.library_paths)

        # Since GetAssets jobs may run before GetAllAssets finishes populating the Asset Index,
        # invalid asset queries can be cached. This process cleans up the cache
        # to ensure only valid entries remain.
        api_rc._asset_index.clean_up_invalid_queries()
