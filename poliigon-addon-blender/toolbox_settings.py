...
from .modules.poliigon_core.user import PoliigonUser, UserDownloadPreferences
...
def get_settings_section_map_prefs(cTB) -> None:
    """ Go through saved settings for map prefs, and set them inside Blender """
    ...
    prefs = get_prefs()
    map_pref_props_arr = prefs.map_prefs
    map_pref_props_arr.clear()  # clear existing entries

    for _map_format in user_prefs.texture_maps:
        map_type_eff: MapType = _map_format.map_type.get_effective()
        new_pref = map_pref_props_arr.add()
        new_pref.name = map_type_eff.name

        map_name: str = _map_format.map_type.name
        local_ext_pref = config.get("map_preferences", str(map_name), fallback="")
        ...
        if local_ext_pref in [None, ""] or not local_ext_available:
            ...
            new_pref.enabled = False
        else:
            _map_format.selected = local_ext_pref.lower()
            _map_format.enabled = True
            new_pref.file_format = local_ext_pref.lower()
            new_pref.enabled = True
    ...
    with cTB.lock_settings_file:
        try:
            ...
            if e.errno == 13:  # EACCES
                error_str = f"{e.strerror}. {_t('Try restarting Blender as administrator.')}"
            elif e.errno == 28:  # ENOSPC
                error_str = f"{e.strerror}. {_t('Free up disk space and try again.')}"
            else:
                error_str = e.strerror
            build_writing_settings_failed_notification(cTB, error_str)
