...
def _build_failed_to_load_asset_data(cTB) -> None:
    """Builds the 'failed to load assets' message."""

    box_not_found = cTB.vBase.box()

    label = _t("Failed to load asset data.")

    width = cTB.width_draw_ui - 20 * get_ui_scale(cTB)
    wrapped_label(cTB, width, label, box_not_found, add_padding=True)

    row = box_not_found.row(align=True)
    row.operator("poliigon.refresh_data", text="Try Again", icon="FILE_REFRESH")
...
def determine_hdri_sizes(asset_data: AssetData,
    ...
    size_bg = None
    if len(list_sizes_light) > 0:
        size_bg_res = find_closest_size(size_bg_default, list_sizes_light)
        if size_bg_res in mapping_size_to_label:
            label = mapping_size_to_label[size_bg_res]
            size_bg = f"{size_bg_res}_{label}"
...
def _draw_button_hdri_local(cTB,
    ...
    size_light, size_bg = determine_hdri_sizes(
        ...
        size_bg_default=size_default,  # Use same as light
...
def _build_unlimited_banner(cTB, layout: bpy.types.UILayout) -> None:
    ...
    text_info = _t("Unlimited Downloads - "
                   "Assets downloaded on an unlimited plan won’t show up as "
                   "Owned.")
...
def _build_tab_title(cTB, dummy_assets_grid: bool = False) -> None:
    ...
    # Right side
    row_right = row.row(align=True)
    row_right.enabled = not cTB.all_assets_failed
...
def build_assets(cTB):
    ...
    if cTB.all_assets_failed:
        _build_failed_to_load_asset_data(cTB)
        return
